...
export default function Dashboard() {
  ...
  const [policyComments, setPolicyComments] = useState<Record<string, string>>({});
  ...
  const handleCommentSubmit = () => {
    if (commentDialog.proposalNo && comment.trim()) {
      setPolicyComments(prev => ({
        ...prev,
        [commentDialog.proposalNo!]: comment
      }));
    }
    setComment("");
    setCommentDialog({
  ...
  return <div className="space-y-6 animate-in">
      ...
      <Card>
        <CardHeader>
          ...
          <div className="flex gap-4 mt-4">
            <Select value={mailboxFilter} onValueChange={(value: "inbox" | "sent") => setMailboxFilter(value)}>
              <SelectTrigger className="w-[180px]">
                <SelectValue placeholder="Select mailbox" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="inbox">Inbox</SelectItem>
                <SelectItem value="sent">Sent To</SelectItem>
              </SelectContent>
            </Select>
          ...
          <Table>
            ...
            <TableBody>
              {filteredPolicies.map(policy => <TableRow key={policy.proposalNo}>
                  ...
                  <TableCell>
                    <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => setCommentDialog({
                  open: true,
        ...
        <div className="flex justify-end gap-2">
          <Button 
            ...
            onClick={() => {
              ...
              selectedRows.forEach(proposalNo => {
                newStatuses[proposalNo] = "acknowledge";
              });
              setPolicyStatuses(newStatuses);
              setSelectedRows([]);
            }} 
      ...
      {/* Comment Dialog */}
      <Dialog open={commentDialog.open} onOpenChange={open => setCommentDialog({
      open,
      proposalNo: null
    })}>
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <Textarea id="comment" placeholder="Enter your comment here..." value={comment} onChange={e => setComment(e.target.value)} rows={5} />
            </div>
          </div>
          <DialogFooter>
